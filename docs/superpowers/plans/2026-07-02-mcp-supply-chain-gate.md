...
- **Malformed-lock hardening (applied during final review):** `_per_server_status` now guards the committed-lock entry with `isinstance(entry, dict)` before calling `diff_lock`. A committed `mcp-lock.json` whose per-server entry is a non-dict (e.g. a bare string) passes `load_lock`'s top-level shape check but previously raised `AttributeError` in `diff_lock` — crashing the standalone CLI and blocking `--update-lock` from self-healing such a repo (the assessor path was already safe via its `try/except`). A malformed entry is now treated like a missing one (`SUP-012` → should-fix, "regenerate the lock"). Regression test: `test_malformed_lock_entry_does_not_crash_and_flags_should_fix`.
