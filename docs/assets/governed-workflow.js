(function () {
  ...
  const definitions = {
    ...
    'confirmation-pending': { actor: 'Gateway + confirmation service', actors: ['gateway', 'control'],
      title: 'Wait without acting', action: 'Save the exact proposal and send a native Outlook approval email to the registered user',
    ...
    confirm: { actor: 'Requesting user', actors: ['user'], title: 'User decides separately',
      action: 'Original requesting user chooses Approve or Reject for the exact proposal in Outlook',
    ...
    'confirmation-verify': { actor: 'Confirmation service', actors: ['control'], title: 'Verify the user decision',
      action: 'Independently verify the native response, original requesting user, exact proposal and deadline', output: '', icon: 'policy' },
  };
  const legacyRoutes = {
  ...
  const narrationClips = {
    ...
    'confirmation-pending': 'The service saves the proposal and sends the user an Outlook approval email. No business change has happened.',
    confirm: 'We pause for the requesting user to choose Approve or Reject in Outlook. Viewing the message never confirms.',
