...
*Generated by `threadlight-production-ready` v0.11.0 at 2026-08-19T18:09:24Z*

## 1. Executive summary

- **Go-live recommendation:** 🔴 NOT READY
- **Raw score:** 29%   **With waivers:** 29%
- **Verification coverage:** 85/170 checks verified (50%) — the rest are `not-verified`
...
- **Not-verified findings:** 85 (live probes that could not run — see Appendix)
- **Verification debt:** 67 not-verified findings (top pillars: sre-handover=9, model-lifecycle=9, secrets=7). `not-verified` no longer earns partial score credit in v0.3.0.
...
❌ **Would fail a hard gate.** 19 must-fix finding(s):
...
- `SUP-010` (supply-chain): MCP servers pinned to an exact version or image digest
...
| 2. Agent governance (AGT) | 🟡 amber | 🟡 amber | 57% | 57% |
| 3. Identity and access | 🔴 red | 🔴 red | 40% | 40% |
| 4. Secrets | 🔴 red | 🔴 red | 19% | 19% |
| 5. Observability | 🔴 red | 🔴 red | 23% | 23% |
| 6. Continuous evals | 🔴 red | 🔴 red | 50% | 50% |
| 7. Responsible AI | 🔴 red | 🔴 red | 42% | 42% |
| 8. HITL and audit | 🔴 red | 🔴 red | 47% | 47% |
| 9. Supply chain | 🔴 red | 🔴 red | 30% | 30% |
| 10. Cost | 🔴 red | 🔴 red | 17% | 17% |
| 11. Reliability | 🔴 red | 🔴 red | 5% | 5% |
| 12. SRE handover | 🔴 red | 🔴 red | 5% | 5% |
| 13. Model lifecycle | 🟡 amber | 🟡 amber | 33% | 33% |
...
| Finding | Severity | Status | Detail |
|---|---|---|---|
| `AGT-001` | must-fix | ❓ not-verified | threadlight-govern manifest (verdict: partial, 43.2d old): capability 'policy_schema_valid' not reported |
| `AGT-002` | must-fix | ✅ pass | threadlight-govern manifest (verdict: partial, 43.2d old): policy_artefact_present=pass |
| `AGT-003` | should-fix | ✅ pass | threadlight-govern manifest (verdict: partial, 43.2d old): asi_reference_present=pass |
| `AGT-004` | should-fix | ✅ pass | threadlight-govern manifest (verdict: partial, 43.2d old): policy_versioned=pass |
| `AGT-005` | should-fix | ❓ not-verified | threadlight-govern manifest (verdict: partial, 43.2d old): capability 'ci_gate_present' not reported |
...
| `GRD-001` | must-fix | ❓ not-verified | Run the producing leg to create specs/ground-manifest.json. |
...
| `RAI-002` | must-fix | ✅ pass | AGT policy constrains sensitive actions |
...
| `GRD-002` | must-fix | ❓ not-verified | Run the producing leg to create specs/ground-manifest.json. |
| `GRD-003` | must-fix | ❓ not-verified | Run the producing leg to create specs/ground-manifest.json. |
| `GRD-004` | should-fix | ❓ not-verified | Run the producing leg to create specs/ground-manifest.json. |
...
| `HITL-006` | must-fix | ✅ pass | 4 skill contract(s) declare a substantive idempotency statement |
| `HITL-007` | should-fix | ✅ pass | SPEC sec 8 names a resume trigger and the state it rehydrates |
...
| `SUP-010` | must-fix | ❌ must-fix | Pin every MCP server to an exact version or image digest: returns-triage-mock |
| `SUP-011` | should-fix | ⚠️ should-fix | Resolve these MCP servers from a known registry/source: returns-triage-mock |
| `SUP-012` | must-fix | ⚠️ should-fix | Commit an mcp-lock.json (run mcp_sbom.py --update-lock) so server/tool drift is reviewable. |
| `SUP-013` | must-fix | ✅ pass | No MCP server commits credentials inline. |
| `INT-001` | must-fix | ❓ not-verified | Run the producing leg to create specs/connect-manifest.json. |
| `INT-002` | must-fix | ❓ not-verified | Run the producing leg to create specs/connect-manifest.json. |
| `UPG-003` | should-fix | ❓ not-verified | Run the producing leg to create specs/upgrade-manifest.json. |
...
| Finding | Severity | Status | Detail |
|---|---|---|---|
| `COST-102` | should-fix | ❓ not-verified | No provable specs/cost-reconciliation-manifest.json — absent, unparseable, wrong schema, or no longer matching the forecast / actuals / SPEC § 14 digests it was reconciled from. Run `threadlight-consumption-iq actuals` to collect Cost Management evidence, then `reconcile` to republish specs/cost-reconciliation-manifest.json. (tier: T3 Cost Management Reader) |
| `COST-103` | should-fix | ❓ not-verified | No provable specs/cost-reconciliation-manifest.json — absent, unparseable, wrong schema, or no longer matching the forecast / actuals / SPEC § 14 digests it was reconciled from. Run `threadlight-consumption-iq actuals` to collect Cost Management evidence, then `reconcile` to republish specs/cost-reconciliation-manifest.json. (tier: T3 Cost Management Reader) |
...
| `COST-007` | must-fix | ❓ not-verified | No vNext meter_coverage in specs/cost-manifest.json — run threadlight-consumption-iq (cost_api) to project meter coverage. |
| `LOAD-002` | should-fix | ❓ not-verified | Run the producing leg to create specs/load-manifest.json. |
...
| `INT-003` | should-fix | ❓ not-verified | Run the producing leg to create specs/connect-manifest.json. |
| `INT-004` | should-fix | ❓ not-verified | Run the producing leg to create specs/connect-manifest.json. |
| `LOAD-001` | must-fix | ❓ not-verified | Run the producing leg to create specs/load-manifest.json. |
| `LOAD-003` | should-fix | ❓ not-verified | Run the producing leg to create specs/load-manifest.json. |
...
| Finding | Severity | Status | Detail |
|---|---|---|---|
| `MDL-001` | must-fix | ❓ not-verified | 1 model deployment(s) set model/version via ARM parameters or expressions (["[format('{0}/{1}', variables('foundryAccountName'), parameters('deployments')[copyIndex()].name)]"]) — pin not statically resolvable; verified at deploy by live check MDL-101 |
...
| `UPG-001` | should-fix | ❓ not-verified | Run the producing leg to create specs/upgrade-manifest.json. |
| `UPG-002` | must-fix | ❓ not-verified | Run the producing leg to create specs/upgrade-manifest.json. |
...
19. **SUP-010** — MCP servers pinned to an exact version or image digest. See: `azd-patterns`
20. **COST-003** — Cost owner documented. See: `paygo-ptu-cost-analyzer`
21. **COST-004** — Idle scale-down configured for ACA / Functions. See: `paygo-ptu-cost-analyzer`
22. **COST-005** — Cost-projection artefact present and fresh (docs/cost-projection.md + specs/cost-manifest.json). See: `paygo-ptu-cost-analyzer`
23. **HITL-005** — HITL decision SLA documented. See: `threadlight-hitl-patterns`
24. **IAM-005** — ACA / Functions auth enabled. See: `foundry-hosted-agents`, `azure-tenant-isolation`
25. **KPI-001** — Outcome KPI baselines declared (latency, cost/interaction, success-rate). See: `foundry-observability`
26. **KPI-002** — Deviation alert wired for an outcome KPI baseline. See: `foundry-observability`
27. **KPI-003** — Outcome scorecard joins eval pass-rate + actual cost/successful interaction + traces. See: `foundry-observability`
28. **MDL-002** — Deprecation plan referenced in SPEC. See: `foundry-skill-catalog`, `foundry-evals`
29. **MDL-006** — Rate limit handling in code. See: `foundry-skill-catalog`, `foundry-evals`
30. **MDL-008** — Knowledge index refresh cadence declared. See: `foundry-skill-catalog`, `foundry-evals`
31. **MDL-011** — Agent thread retention/policy declared in SPEC. See: `foundry-skill-catalog`, `foundry-evals`
32. **NET-003** — Public network access disabled on AI services. See: `citadel-spoke-onboarding`, `foundry-vnet-deploy`, `foundry-network-runbook`
...
34. **OBS-005** — Workbook scaffold present. See: `foundry-observability`
35. **RAI-005** — Groundedness check planned for RAG. See: `foundry-agt`
...
42. **SRE-003** — Azure SRE Agent integration considered. See: `azure-sre-agent` (recipe: `threadlight-pilot-handover`)
43. **SRE-004** — Severity matrix documented. See: `azure-sre-agent` (recipe: `threadlight-pilot-handover`)
44. **SRE-005** — Postmortem template referenced. See: `azure-sre-agent` (recipe: `threadlight-pilot-handover`)
45. **SUP-004** — SBOM generation step declared. See: `azd-patterns`
46. **SUP-005** — Vulnerability scan step declared. See: `azd-patterns`
47. **SUP-006** — ACR scoped to private network. See: `azd-patterns`
48. **SUP-007** — Provenance / attestation considered. See: `azd-patterns`
49. **SUP-009** — Agent skills/tools pinned to a version for production. See: `azd-patterns`
50. **SUP-011** — MCP servers resolve from a known registry or source. See: `azd-patterns`
51. **SUP-012** — mcp-lock.json committed and free of undocumented drift. See: `azd-patterns`
...
| `AGT-001` | agent-governance | T0 | threadlight-govern manifest (verdict: partial, 43.2d old): capability 'policy_schema_valid' not reported |
| `AGT-005` | agent-governance | T0 | threadlight-govern manifest (verdict: partial, 43.2d old): capability 'ci_gate_present' not reported |
| `AGT-101` | agent-governance | T1 | Skipped — running in --static mode |
| `AGT-102` | agent-governance | T2 | Skipped — running in --static mode |
| `GRD-001` | identity-access | T0 | Run the producing leg to create specs/ground-manifest.json. |
...
| `GRD-002` | responsible-ai | T0 | Run the producing leg to create specs/ground-manifest.json. |
| `GRD-003` | responsible-ai | T0 | Run the producing leg to create specs/ground-manifest.json. |
| `GRD-004` | responsible-ai | T0 | Run the producing leg to create specs/ground-manifest.json. |
...
| `INT-001` | supply-chain | T0 | Run the producing leg to create specs/connect-manifest.json. |
| `INT-002` | supply-chain | T0 | Run the producing leg to create specs/connect-manifest.json. |
| `UPG-003` | supply-chain | T0 | Run the producing leg to create specs/upgrade-manifest.json. |
...
| `COST-102` | cost | T3 | No provable specs/cost-reconciliation-manifest.json — absent, unparseable, wrong schema, or no longer matching the forecast / actuals / SPEC § 14 digests it was reconciled from. Run `threadlight-consumption-iq actuals` to collect Cost Management evidence, then `reconcile` to republish specs/cost-reconciliation-manifest.json. |
| `COST-103` | cost | T3 | No provable specs/cost-reconciliation-manifest.json — absent, unparseable, wrong schema, or no longer matching the forecast / actuals / SPEC § 14 digests it was reconciled from. Run `threadlight-consumption-iq actuals` to collect Cost Management evidence, then `reconcile` to republish specs/cost-reconciliation-manifest.json. |
| `COST-006` | cost | T0 | Run threadlight-consumption-iq to populate cost-manifest.json before scoring COST-006. |
| `COST-007` | cost | T0 | No vNext meter_coverage in specs/cost-manifest.json — run threadlight-consumption-iq (cost_api) to project meter coverage. |
| `LOAD-002` | cost | T0 | Run the producing leg to create specs/load-manifest.json. |
...
| `INT-003` | reliability | T0 | Run the producing leg to create specs/connect-manifest.json. |
| `INT-004` | reliability | T0 | Run the producing leg to create specs/connect-manifest.json. |
| `LOAD-001` | reliability | T0 | Run the producing leg to create specs/load-manifest.json. |
| `LOAD-003` | reliability | T0 | Run the producing leg to create specs/load-manifest.json. |
...
| `MDL-001` | model-lifecycle | T0 | 1 model deployment(s) set model/version via ARM parameters or expressions (["[format('{0}/{1}', variables('foundryAccountName'), parameters('deployments')[copyIndex()].name)]"]) — pin not statically resolvable; verified at deploy by live check MDL-101 |
| `UPG-001` | model-lifecycle | T0 | Run the producing leg to create specs/upgrade-manifest.json. |
| `UPG-002` | model-lifecycle | T0 | Run the producing leg to create specs/upgrade-manifest.json. |
