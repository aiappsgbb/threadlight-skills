...
nonproduction MAF `hosted_cohort: {file, sha256}` input: a retained canonical
`pyproject.toml`, at most 32 KiB, with its actual prefixed SHA-256. It validates
the MAF/Projects family and copies only the three exact Agent Server pins.
Frozen output records the hash/pins without operator paths; global pins are not
changed. This compatibility selection is not proof for an unexecuted image.
That runbook also covers the opt-in read audit, `returns_reconcile.py` and the
disabled `review-notification.bicep` module. Human consent and grants are never
generated by those helpers.
