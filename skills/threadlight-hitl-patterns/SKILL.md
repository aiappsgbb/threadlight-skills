...
- `specs/SPEC.md` § 8 — for each interaction:
  - `Gate ID`: stable `GATE-NNN` identifier for the SPEC interaction
...
### Governed conditional tool extension

When a gate releases a **governed conditional tool** from an enabled manifest,
extend the input/output contract additively:

- `gate` remains the canonical UX taxonomy (`approve`, `reject`, etc.);
  `gate_id` is the stable SPEC interaction id (`GATE-NNN`). Preserve both.
- Create `correlation_id` before policy evaluation and preserve it through the
  card payload, Action.Submit round-trip, approval persistence, and runtime
  adapter call.
- Generate `approval_id` only when the approval audit record is persisted, then
  return it together with the original `correlation_id`.
- Copy `policy_id`, `tool_name`, and `contract_sha256` from the enabled
  `tool_governance` manifest for the governed tool the gate can release.
- These governed fields are required only when a gate-backed tool is
  conditional; the seven canonical gates and the legacy audit shape remain
  unchanged for ungoverned flows.
...
- For governed conditional tools: include `gate_id`, `correlation_id`,
  `policy_id`, `tool_name`, and `contract_sha256` in the card/action payload so
  the same identifiers survive the full approval path
...
async def handle(turn_context: TurnContext,
    ...
    Args:
        ...
        value:        activity.value parsed dict (gate, gate_id, case_id,
                      correlation_id, decision, edits, ...).
    ...
    gate_id = value.get("gate_id")        # stable SPEC interaction id (GATE-NNN)
    ...
    # 2. If this gate releases a governed conditional tool, persist approval audit
    #    event before releasing the governed tool.
    # 3. approval_id is generated by that persisted approval write, then the
    #    handler passes gate_id, correlation_id, and approval_id to the runtime
    #    adapter. A gate-backed tool executes only after matching approval persisted.
    # 4. same approval_id cannot execute twice; replay returns the prior outcome.
    # 5. Write audit trail (gate, gate_id, decision, actor, timestamp,
    #    linked_rules, correlation_id, approval_id, policy_id, tool_name,
    #    contract_sha256, ...)
    # 6. Return updated card (success or error message)
...
>
> For governed conditional tools, persist approval audit event before releasing the governed tool.
>
> For governed conditional tools, the runtime adapter must treat `approval_id`
> as the idempotency key: a matching replay returns the prior outcome instead of
> executing again.
...
For ungoverned gates, keep writing the existing canonical fields (`case_id`,
`gate`, `decision`, `actor`, `timestamp`, `linked_rules`, ...) and treat the
governed additions as absent. For governed conditional tools, those additive
fields become required because the audit row must prove which manifest contract
released which tool call.
