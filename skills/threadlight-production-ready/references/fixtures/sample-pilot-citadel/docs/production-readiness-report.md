...
*Generated by `threadlight-production-ready` v0.11.0 at 2026-08-19T18:09:11Z*

## 1. Executive summary

- **Go-live recommendation:** 🔴 NOT READY
- **Raw score:** 43%   **With waivers:** 43%
- **Verification coverage:** 86/169 checks verified (50%) — the rest are `not-verified`
...
- **Not-verified findings:** 83 (live probes that could not run — see Appendix)
...
- ⚠️ `COST-004` (cost) — Idle scale-down configured for ACA / Functions. No scale-to-zero / idle scale-down in compute
...
|---|---|---|---|---|
| 1. Network posture | 🟡 amber | 🟡 amber | 44% | 44% |
| 2. Agent governance (AGT) | 🟢 green | 🟢 green | 85% | 85% |
| 3. Identity and access | 🟡 amber | 🟡 amber | 56% | 56% |
| 4. Secrets | 🟡 amber | 🟡 amber | 39% | 39% |
| 5. Observability | 🟡 amber | 🟡 amber | 32% | 32% |
| 6. Continuous evals | 🔴 red | 🔴 red | 62% | 62% |
| 7. Responsible AI | 🟡 amber | 🟡 amber | 45% | 45% |
| 8. HITL and audit | 🔴 red | 🔴 red | 30% | 30% |
| 9. Supply chain | 🔴 red | 🔴 red | 28% | 28% |
| 10. Cost | 🟡 amber | 🟡 amber | 35% | 35% |
| 11. Reliability | 🟡 amber | 🟡 amber | 39% | 39% |
| 12. SRE handover | 🟡 amber | 🟡 amber | 30% | 30% |
| 13. Model lifecycle | 🔴 red | 🔴 red | 20% | 20% |
...
| Finding | Severity | Status | Detail |
|---|---|---|---|
| `AGT-001` | must-fix | ✅ pass | AGT policy has version/name/rules — lints clean |
| `AGT-002` | must-fix | ✅ pass | policy.yaml present |
| `AGT-003` | should-fix | ✅ pass | OWASP ASI 2026 referenced |
| `AGT-004` | should-fix | ✅ pass | AGT policy ruleset version pinned |
| `AGT-005` | should-fix | ✅ pass | AGT governance gate runs in CI |
...
| `IAM-006` | must-fix | ✅ pass | All agent identities are passwordless (managed/federated). |
| `IAM-007` | should-fix | ⚠️ should-fix | Declare a responsible owner (tag owner/ownerEmail/ManagedBy or agent-identity.governance.json) for: uami-pilot-citadel |
| `IAM-008` | must-fix | ⚠️ should-fix | Agent identity scope is least-privilege (no Owner/Contributor/wildcard). |
| `IAM-009` | should-fix | ⚠️ should-fix | Declare a lifecycle (reviewBy/expiresOn tag or a federated credential with no standing secret) for: uami-pilot-citadel |
...
| `HITL-006` | must-fix | ❓ not-verified | no skill contracts under src/agent/skills/ to inspect |
| `HITL-007` | should-fix | ⚠️ should-fix | SPEC sec 8 names no resume trigger and no rehydrated state — the gate cannot outlive the session |
...
| `REL-007` | must-fix | ✅ pass | Restore-drill artefact `restore-drill-2026-05-11.md` is 6 days old (≤90d) |
...
| `HITL-006` | hitl-audit | T0 | no skill contracts under src/agent/skills/ to inspect |
...
- safe-check manifest is 1682.0h old (limit 24h) — accepted via --accept-stale-safe-check
