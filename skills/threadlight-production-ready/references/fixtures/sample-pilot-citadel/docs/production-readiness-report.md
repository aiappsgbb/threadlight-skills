...
- **Raw score:** 47%   **With waivers:** 47%
- **Verification coverage:** 85/152 checks verified (55%) — the rest are `not-verified`
...
- **Not-verified findings:** 67 (live probes that could not run — see Appendix)
- **Verification debt:** 49 not-verified findings (top pillars: sre-handover=9, secrets=7, model-lifecycle=6). `not-verified` no longer earns partial score credit in v0.3.0.
...
9. **IAM-007** — Agent identity declares a responsible owner. See: `foundry-hosted-agents`, `azure-tenant-isolation`
10. **IAM-008** — Agent identity scope is least-privilege. See: `foundry-hosted-agents`, `azure-tenant-isolation`
...
18. **MDL-006** — Rate limit handling in code. See: `foundry-skill-catalog`, `foundry-evals`
19. **MDL-008** — Knowledge index refresh cadence declared. See: `foundry-skill-catalog`, `foundry-evals`
20. **MDL-009** — Project-level RBAC declared on Foundry account. See: `foundry-skill-catalog`, `foundry-evals`
21. **MDL-011** — Agent thread retention/policy declared in SPEC. See: `foundry-skill-catalog`, `foundry-evals`
22. **OBS-004** — Foundry observability emit configured. See: `foundry-observability`
23. **OBS-005** — Workbook scaffold present. See: `foundry-observability`
24. **RAI-004** — PII redaction strategy documented. See: `foundry-agt`
25. **RAI-005** — Groundedness check planned for RAG. See: `foundry-agt`
26. **REL-002** — Multi-region plan documented if RTO < 4h. See: `foundry-vnet-deploy`, `foundry-caphost-lifecycle`
27. **REL-004** — Capacity host lifecycle understood. See: `foundry-vnet-deploy`, `foundry-caphost-lifecycle`
28. **SEC-003** — appsettings reference KV references, not raw values. See: `azd-patterns`
29. **SEC-004** — Secret rotation policy documented in SPEC. See: `azd-patterns`
30. **SRE-003** — Azure SRE Agent integration considered. See: `azure-sre-agent` (recipe: `threadlight-pilot-handover`)
31. **SUP-004** — SBOM generation step declared. See: `azd-patterns`
32. **SUP-005** — Vulnerability scan step declared. See: `azd-patterns`
33. **SUP-007** — Provenance / attestation considered. See: `azd-patterns`
...
- safe-check manifest is 647.5h old (limit 24h) — accepted via --accept-stale-safe-check
