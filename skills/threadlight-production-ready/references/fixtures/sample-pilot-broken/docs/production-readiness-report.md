...
*Generated by `threadlight-production-ready` v0.11.0 at 2026-08-19T18:15:20Z*

## 1. Executive summary

- **Go-live recommendation:** 🔴 NOT READY
- **Raw score:** 20%   **With waivers:** 20%
- **Verification coverage:** 78/157 checks verified (49%) — the rest are `not-verified`
- **Evidence confidence:** LOW — `HIGH` ≥80%, `MEDIUM` 50–79%, `LOW` <50%. See Appendix for permission-tier breakdown.
...
- **Not-verified findings:** 79 (live probes that could not run — see Appendix)
- **Verification debt:** 62 not-verified findings (top pillars: sre-handover=9, model-lifecycle=8, secrets=7). `not-verified` no longer earns partial score credit in v0.3.0.
...
| `RAI-002` | must-fix | ❌ must-fix | No AGT policy constrains sensitive actions |
...
| `HITL-006` | must-fix | ⚪ not-applicable | No HITL declared in SPEC — no gated terminal write to replay |
| `HITL-007` | should-fix | ⚪ not-applicable | No HITL declared in SPEC — no gate to resume |
...
- safe-check manifest is 1682.2h old (limit 24h) — accepted via --accept-stale-safe-check
