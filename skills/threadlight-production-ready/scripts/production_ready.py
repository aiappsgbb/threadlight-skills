...
def _scaffold_cicd(framing: dict, repo_full_name: str, out_root) -> list:
    """Use the authoritative verified-release generator for every handoff."""
    import importlib.util
    generator_path = Path(__file__).resolve().parents[2] / "threadlight-cicd/scripts/generate_pipeline.py"
    if not generator_path.is_file():
        raise ValueError("Install the complete Threadlight catalog: verified-release generator is missing")
    spec = importlib.util.spec_from_file_location("threadlight_verified_release", generator_path)
    generator = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(generator)
    values = dict(framing, repo_full_name=repo_full_name)
    values.setdefault("platform", framing.get("cicd_target", "github-actions"))
    values.setdefault("tenant_id", framing.get("azure_tenant_id", ""))
    values.setdefault("central_env_required", framing.get("target_posture") == "citadel-spoke")
    for old, new in (("central_platform_team_rg", "uami_resource_group"),
                     ("central_platform_team_sub", "uami_subscription_id")):
        if framing.get(old):
            values.setdefault(new, framing[old])
    out_root = Path(out_root)
    written = generator.generate(values, out_root)
    pointer = out_root / "docs/threadlight-cicd/central-team-uami-readme.md"
    pointer.write_text(
        f"# Verified-release handoff for {repo_full_name}\n\n"
        "Use [production setup](env-setup/README.md), "
        "[validation setup](validation-env-setup/README.md) and "
        "the [release contract](release-contract.md). Configure separate identities, "
        "targets, protected environment approvals and real application adapters. "
        "The example policy is not executable authorization.\n", encoding="utf-8",
    )
    written.append(pointer)
    _eprint("Verified-release scaffold generated by threadlight-cicd; configure the reviewed policy before enabling it.")
    return written
...
def _hint_pipeline_scaffold_if_needed(apply_plan: dict, scaffold_cicd_flag: bool) -> None:
    ...
    _eprint(
        ...
        f"Re-run with --scaffold-cicd to generate the selected verified-release pipeline and environment runbooks."
    )
...
FRAMING_QUESTIONS = [
    ...
    {
        "id": "cicd_target",
        "prompt": "Verified-release CI/CD target?",
        "kind": "choice",
...
def _render_report(manifest: dict, posture: dict, pillar_results_waived: dict[str, list[Finding]],
    ...
    if kpi:
        ...
        out.append("Joins the three outcome signals a production review needs (eval quality + measured unit cost + live telemetry):")
        out.append("")
