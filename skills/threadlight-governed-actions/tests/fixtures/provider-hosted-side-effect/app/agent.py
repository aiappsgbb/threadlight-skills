"""The provider-hosted-side-effect fixture's single declared application module.

Identical to the conformant fixture's module except for its own declared
defect: its action registry additionally declares two exclusively
provider-hosted tools (``mail.send``, ``search.lookup``) whose effects
execute inside the provider's own runtime, out of reach of this
repository's pre-action seam; every other seam behaves exactly as the
conformant fixture's does.
...
# This fixture's declared defect. Both actions run exclusively inside the
# provider's own tool runtime, so no dispatch function below can mediate
# them from this repository at all; only ``mail.send`` actually matters,
# because only it carries an externally consequential effect.


@tool(name="mail.send")
def mail_send(recipient: str, body: str) -> dict[str, Any]:
    """Send mail. External egress, executed provider-side."""
    return {"recipient": recipient, "body": body}


@tool(name="search.lookup")
def search_lookup(query: str) -> dict[str, Any]:
    """Search. Read-only, executed provider-side."""
    return {"query": query}
