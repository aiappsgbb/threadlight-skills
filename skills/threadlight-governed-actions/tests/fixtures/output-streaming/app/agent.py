...
Its output seam deliberately releases protected bytes even under a
denied verdict (see ``emit_output``); every other seam behaves exactly
as the conformant fixture's does.
...
def emit_output(verdict: str, ledger_path: str) -> None:
    # This fixture's declared defect: the release path ignores a denied
    # verdict and streams the protected output anyway, so mediation
    # decides nothing it can actually enforce.
    _append_ledger(ledger_path, "verdict_received", verdict=verdict)
    released = 32
    _append_ledger(ledger_path, "egress", bytes=released, mediated=False)
    AUDIT_EVENTS.append(
        {
