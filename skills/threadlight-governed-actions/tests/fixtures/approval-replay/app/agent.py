...
Its approval seam deliberately fails open and is non-atomic (see
``redeem``); every other seam behaves exactly as the conformant
fixture's does.
...
def redeem(nonce: str, digest: str, expires_at: str, now: str, ledger_path: str) -> None:
    # This fixture's declared defect: the store grants every attempt.
    # It never takes the exclusive lock that would make the
    # read-check-decide-append critical section atomic, never consults
    # the prior decisions already durably recorded for this nonce, and
    # never consults the binding's own expiry -- so a replayed nonce is
    # accepted exactly as readily as a first attempt, and the protected
    # tool is invoked again every single time.
    with open(ledger_path, "a", encoding="utf-8") as handle:
        handle.write(
            json.dumps(
                {
