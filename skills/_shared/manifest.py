...
# subsequent parse. ``fullmatch`` below intentionally rejects trailing newlines
# that ``^...$`` would otherwise accept. Mirrors the ``format: date-time``
# semantics the manifest schemas declare and the RFC-3339 backend jsonschema's
# ``FormatChecker`` uses, but stays stdlib-only.
...
def validate_iso8601_timestamp(value, field, *, nullable=False):
...
def validate_envelope(envelope):
    ...
    validate_iso8601_timestamp(envelope["generated_at"], "generated_at")
    ...
    # Draft-07 integer semantics (see is_draft7_integer): an integral float
    ...
    if not is_draft7_integer(valid_for_hours) or valid_for_hours <= 0:
        raise ManifestValidationError(
...
_is_draft7_integer = is_draft7_integer
