...
Gateway and control-plane packages 0.5.0 preserve the existing authority wire
protocol; effective generation hashes bind new contracts without rewriting old grants.
...
Portable reference version **0.5.0** adds `POST /governance/operations`, outside
...
For an operation originally bound to a requesting user, include its original
opaque `requesting_user_context` in action inspection and reconciliation.
It must reproduce the original input hash; it is not fresh consent. Recovery
does not consume confirmation or reviewer authority again. Pending human
decisions remain pending: only an execution reservation with its pre-effect
central audit receipt can be reconciled.

If the action requires signed evidence, reconciliation also requires a current
`evidence_token` with the original semantic fingerprint. The gateway rechecks
the native policy and trusted evidence without retaining the token or claims.
Inspection and completed-outcome retrieval do not require token renewal.
Citadel operations retain the effective binding and both owner-policy digests,
and check the generation and both policy authorities. Admission is bound to the
workload/action/deployment, not to one transient business-evidence fingerprint.
Stop is rechecked after confirmation and terminal policy waits, before effects.
...
Ordinary effects and retrieval use registered `POST endpoint` and
`GET outcome_endpoint`; opted-in recovery uses the separate contract above:
