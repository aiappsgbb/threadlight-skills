...
def configure_operator(h):
    h.cp.settings.operation_controllers.update({
        HUMAN: cp("auth").ProbeController(client_id=UI, subjects=[WORKLOAD], actions=["refund"])})
...
@pytest.mark.parametrize("fields", [
    {"recovery_contract": None}, {"recovery_endpoint": None},
    {"recovery_contract": None, "recovery_endpoint": None},
])
def test_explicit_null_recovery_is_not_unselected(fields):
    with pytest.raises(ValueError, match="recovery"):
        gateway("dispatcher").Action.model_validate({**registry()["actions"][0], **fields})


@pytest.mark.governance_runtime
def test_operator_admission_coexists_with_signed_evidence(tmp_path):
    async def check():
        from test_evidence_gateway import harness as evidence_harness
        h, _, _, _, _, arguments, _, _, token = await evidence_harness(tmp_path)
        configure_operator(h)
        try:
            opened = await open_admission(h)
            completed = await h.call(arguments=arguments, evidence_token=token)
            assert completed["status"] == "completed", completed
            inspected = await operate(h, "inspect", operation_id="one", arguments=arguments)
            assert inspected["state"] == "completed", inspected
            await operate(h, "admission", expected_record_hash=opened["record_hash"],
                          state="stopped", reason_code="incident")
            assert await h.call(arguments=arguments) == completed
...
@pytest.mark.governance_runtime
def test_operator_confirmation_inspection_and_stop_preserve_original_context(tmp_path):
    async def check():
        from test_confirmation_gateway import harness as confirmation_harness, call, decide
        h = await confirmation_harness(tmp_path)
        configure_operator(h)
        try:
            opened = await open_admission(h)
            ...
            inspected = await operate(h, "inspect", operation_id="one", arguments={"amount": 5},
                                      requesting_user_context=h.context_ref)
            assert inspected["state"] == "awaiting_confirmation", inspected
            await decide(h, pending)
            await operate(h, "admission", expected_record_hash=opened["record_hash"],
                          state="stopped", reason_code="incident")
            assert (await call(h))["reason_code"] == "admission_closed"
            assert not h.calls
        finally:
...
@pytest.mark.governance_runtime
@pytest.mark.parametrize("stop_during_policy", [False, True])
def test_operator_citadel_admission_preserves_dual_policy_and_terminal_stop(tmp_path, stop_during_policy):
    async def check():
        from test_citadel import native_pair
        h = await native_pair(tmp_path, "allow", "allow")
        configure_operator(h)
        try:
            opened = await open_admission(h)
            original = h.policy.evaluate
            evaluations = 0
            async def evaluate(*args, **kwargs):
                nonlocal evaluations
                result = await original(*args, **kwargs)
                evaluations += 1
                if stop_during_policy and evaluations == 2:
                    stopped = await operate(h, "admission", expected_record_hash=opened["record_hash"],
                                            state="stopped", reason_code="terminal_stop")
                    assert stopped["state"] == "stopped", stopped
                return result
            h.policy.evaluate = evaluate
            result = await h.call()
            if stop_during_policy:
                assert result["reason_code"] == "admission_closed", result
                assert not h.calls
            else:
...
@pytest.mark.governance_runtime
@pytest.mark.parametrize("profile", ["evidence", "confirmation", "both", "citadel"])
def test_operator_recovery_preserves_new_authorities_without_redispatch(tmp_path, profile):
    async def check():
        from test_confirmation_gateway import harness as confirmation_harness, decide
        from test_evidence_attestations import fixture
        from test_citadel import native_pair, contract
        recovery = {"recovery_contract": "fenced-outcome/v1",
                    "recovery_endpoint": "https://downstream.example/recover"}
        arguments, token = {"amount": 5}, None
        document = registry()
        document["actions"][0].update(recovery)
        if profile in {"evidence", "both"}:
            _, _, requirement, identity, arguments, _, _, provider, _ = fixture()
            ...
            token = (await provider.issue(identity, arguments))["attestation"]
        if profile in {"confirmation", "both"}:
            h = await confirmation_harness(tmp_path, document=document)
        elif profile == "citadel":
            binding = contract()
            for owner in ("producer", "consumer"):
                binding[owner]["action"].update(recovery)
            h = await native_pair(tmp_path, "allow", "allow", document=binding)
        else:
            h = await GatewayHarness().initialize(tmp_path, document=document)
        configure_operator(h)
        context = ({"requesting_user_context": h.context_ref}
                   if profile in {"confirmation", "both"} else {})
        try:
            await open_admission(h)
            call_arguments = {"arguments": arguments, "evidence_token": token, **context}
            if context:
                pending = await h.call(**call_arguments)
                assert pending["status"] == "pending_confirmation", pending
                await decide(h, pending)
            h.downstream_status = 503
            assert (await h.call(**call_arguments))["status"] == "unavailable"
            inspected = await operate(h, "inspect", operation_id="one", arguments=arguments, **context)
            assert inspected["state"] == "pending", inspected
            before = deepcopy(h.store.docs)
            if token is not None:
                missing = await operate(h, "reconcile", operation_id="one", arguments=arguments,
                                        expected_record_hash=inspected["record_hash"], **context)
                assert missing["reason_code"] == "evidence_required", missing
                assert h.store.docs == before
            async def witness(request):
                assert request.url.path == "/recover"
                return httpx.Response(200, json={
                    "state": "not_executed", "receipt_id": "fence-1",
                    "action_hash": request.headers["x-action-hash"],
                    "provenance": request.headers["x-governance-provenance"],
                    "operation_key": request.headers["idempotency-key"],
                    "deployment_hash": request.headers["x-deployment-hash"]})
            ...
            h.dispatcher.downstream = gateway("dispatcher").DownstreamClient(
                credential=h.credential, transport=httpx.MockTransport(witness))
            result = await operate(h, "reconcile", operation_id="one", arguments=arguments,
                                   expected_record_hash=inspected["record_hash"],
                                   evidence_token=token, **context)
            assert result["state"] == "not_executed", result
            assert (await h.call(**call_arguments))["reason_code"] == "operation_fenced"
            assert len(h.calls) == 1
            await h.dispatcher.downstream.aclose()
        finally:
            if context:
...
@pytest.mark.governance_runtime
def test_terminal_operator_reconciliation_rechecks_authority_after_storage_wait(tmp_path):
    async def check():
        h = await harness(tmp_path)
        try:
            await open_admission(h)
            assert (await h.call())["status"] == "completed"
            inspected = await operate(h, "inspect", operation_id="one", arguments={"amount": 5})
            original = h.store.read
            async def revoked(*args, **kwargs):
                value = await original(*args, **kwargs)
                h.cp.settings.operation_controllers.clear()
                return value
            h.store.read = revoked
            result = await operate(h, "reconcile", operation_id="one", arguments={"amount": 5},
                                   expected_record_hash=inspected["record_hash"])
            assert result.get("reason_code") == "operator_unauthorized", result
            assert len(h.calls) == 1
        finally:
            await h.close()
    asyncio.run(check())
